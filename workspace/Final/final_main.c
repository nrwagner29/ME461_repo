...
// #############################
// ###  ROBOT WHEEL CONTROL  ###     (DC MOTORS) , (EPWM2)
// #############################
...
// #################################
// ###  ROBOT MECHANISM CONTROL  ###     (SERVOMOTORS) , (EPWM8)
// #################################
...
__interrupt void SWI_isr(void)
{
    ...
    case 10: //standing
        standcmd=0;
        setEPWM8A(SERVO_stand);// store arms *will this be active long enough to store arms fully?*
        setEPWM8B(-SERVO_stand);

        if (balcmd == 1) {
            //activate arm

            setEPWM8A(SERVO_swing);// store arms *will this be active long enough to store arms fully?*
            setEPWM8B(-SERVO_swing);

            countcase10 -= 0.1;
            if (countcase10 < SERVO_tipped) {
                countcase10 = SERVO_tipped;
                myStateVar = 15; //Tipped
            }
            setEPWM8A(countcase10);// store arms *will this be active long enough to store arms fully?*
            setEPWM8B(-countcase10);
        }
        //
        if (rovercmd == 1) {
            //ZEROING
            ...
            countcase10_2 += 0.1;
            if (countcase10_2 >= SERVO_rover) {
                countcase10_2 = SERVO_rover;
            }
            setEPWM8A(countcase10_2);// store arms *will this be active long enough to store arms fully?*
            setEPWM8B(-countcase10_2);
    ...
    case 20://balancing
        ...
        if(standcmd  == 1) {
        ...
        else if (rovercmd == 1) { // IMPROVE

            //ZEROING

            countcase20_3 ++;
            if (countcase20_3 < 375){
                actbalance = 0;
                setEPWM8A(SERVO_stand);// store arms *will this be active long enough to store arms fully?*
                setEPWM8B(-SERVO_stand);
            }
            if (countcase20_3 >= 375){
                countcase20_2 += 0.1;
                if (countcase20_2 >= SERVO_rover) {
                    countcase20_2 = SERVO_rover;
                }
                setEPWM8A(countcase20_2);// store arms *will this be active long enough to store arms fully?*
                setEPWM8B(-countcase20_2);
    ...
    case 30://rover
        ...
        if (balcmd == 1) {
            countcase30 -= 0.1;
            if (countcase30 < SERVO_tipped) {
                countcase30 = SERVO_tipped;
            }
            setEPWM8A(countcase30);// store arms *will this be active long enough to store arms fully?*
            setEPWM8B(-countcase30);

            //activate arm
            if (countcase30 <= SERVO_tipped){
                actrover = 0;
                myStateVar = 15; //Balanced
            }
        }
        if(standcmd  == 1) {
            countcase30_2 -= 0.1;
            if (countcase30_2 < SERVO_stand) {
                countcase30_2 = SERVO_stand;
            }
            setEPWM8A(countcase30_2);// store arms *will this be active long enough to store arms fully?*
            setEPWM8B(-countcase30_2);

            //activate arm
            if (countcase30_2 <= SERVO_stand){
                actrover = 0;
                myStateVar = 10; //Balanced
                countcase30_2 = 70;
            }
        }
        break;
    ...
    if (actbalance == 1 & actrover == 0){
        setEPWM2A(uRight);
        setEPWM2B(-uLeft);
    }
    if (actbalance == 0 & actrover == 0){
        setEPWM2A(0);
        setEPWM2B(0);
    }
    ...
    //errL = Segbot_refSpeed - VLeftK;
    //errR = Segbot_refSpeed - VRightK;
    errL = Segbot_refSpeed -VLeftK-KPturn*eturn;
    errR = Segbot_refSpeed -VRightK+KPturn*eturn;
    ...
    //input
    uRLeft = kp*errL + ki*LI;
    uRRight = kp*errR + ki*RI;

    //Set EPWM to control input
    if (actrover == 1 & actbalance == 0){
        setEPWM2A(uRRight);
        setEPWM2B(uRLeft);
    }
    ...
    if (NewLVData == 1) {
        ...
        rovercmd = fromLVvalues[2];
        balcmd = fromLVvalues[3];
        standcmd = fromLVvalues[4];
