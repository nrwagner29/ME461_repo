...
void main(void)
{
    ...
    ConfigCpuTimer(&CpuTimer2, LAUNCHPAD_CPU_FREQUENCY, 250000); //The max limit = 21.475sec = 21.--- million microseconds
    ...
    while(1)
    {
        ...
        if (UARTPrint == 1 ) {
            serial_printf(&SerialA,"Num Timer2:%ld Num SerialRX: %ld switchread:%d\r\n",numTimer2calls,numRXA,swrow);
            UARTPrint = 0;
        }
    }
}
...
__interrupt void cpu_timer2_isr(void)
{
    ...
    UARTPrint = 1;
    swrow = ReadSwitches();
//    SetLEDRowsOnOff(swrow);
    SetLEDRowsOnOff(CpuTimer2.InterruptCount);
